    ...
    for epoch in range(opt.num_epochs):
        ...
        if opt.region_relation == 'NTN_relation':
            save_checkpoint({
                ...
                'opt': opt,
                'Eiters': model.Eiters,
            }, is_best, filename='checkpoint_{}.pth.tar'.format(epoch%5), prefix=opt.model_name + '/' + opt.region_relation + '/')
...
def validate(opt, val_loader, model):
    ...
    file_result = open(opt.result_name + '/' + '%s_%s_%s_result.txt' %(opt.data_name, opt.Matching_direction, opt.Add_features),'a')
