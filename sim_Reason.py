...
class Encoding_sim(nn.Module):
    def __init__(self, embed_size):
        ...
        self.transfer = nn.Linear(self.embed_dim, self.embed_dim)
        self.init_weights()
    ...
    def forward(self, nodes1, nodes2, num_block, xlambda, Global_information=True):
        ...
        transfer_features = self.transfer(nodes1)
        featureT = torch.transpose(transfer_features, 1, 2)
        ...
        loc_mvector = cosine_similarity(qry_set, ctx_set, dim=-1)

        if Global_information == True:
            global_nodes1 = torch.mean(weightFeatureT, dim=1).view(batch_size, num_block, -1)
            global_nodes2 = torch.mean(nodes2, dim=1).view(batch_size, num_block, -1)
            glo_mvector = cosine_similarity(global_nodes2, global_nodes1, dim=-1).unsqueeze(1)
            sim_mvector = torch.cat((loc_mvector, glo_mvector), dim=1)
        else:
            sim_mvector = loc_mvector
        return sim_mvector
...
class Reason_i2t(nn.Module):
    ...
    def __init__(self,
        ...
        # i2t feature transfer
        self.i2t_transfer = nn.Linear(opt.embed_size, opt.embed_size)
    ...
    def i2t_scores(self, tnodes, vnodes, num_block):
        ...
        sim_mvector = self.Encoding_sim(tnodes, vnodes, num_block, 9.0, Global_information=self.opt.Global_information)
        ...
        if self.opt.Global_information == True:
            sim = self.out_2(self.mean_out(sim_mvector.view(batch_size * (num_regions + 1), -1)).
                             view(batch_size, (num_regions + 1), -1).tanh())
        else:
            sim = self.out_2(self.mean_out(sim_mvector.view(batch_size * num_regions, -1)).
                             view(batch_size, num_regions, -1).tanh())
    ...
    def forward(self, images, captions, cap_lens, opt):
        ...
        for i in range(n_caption):
            ...
            i2t = self.i2t_scores(
                cap_i_expand, images, opt.num_block)
...
class Reason_t2i(nn.Module):
    ...
    def t2i_scores(self, vnodes, tnodes, num_block):
        ...
        sim_mvector = self.Encoding_sim(vnodes, tnodes, num_block, 20.0, self.opt.Global_information)
        sim_mvector = sim_mvector * self.delt
        if self.opt.Global_information == True:
            sim = self.out_2(self.mean_out(sim_mvector.view(batch_size * (num_words + 1), -1)).
                             view(batch_size, (num_words + 1), -1).tanh())
        else:
            sim = self.out_2(self.mean_out(sim_mvector.view(batch_size * num_words, -1)).
                             view(batch_size, num_words, -1).tanh())
        sim = sim.view(batch_size, -1).mean(dim=1, keepdim=True)
    ...
    def forward(self, images, captions, cap_lens, opt):
        ...
        for i in range(n_caption):
            ...
            cap_i_expand = cap_i.repeat(n_image, 1, 1)
            t2i = self.t2i_scores(
                images, cap_i_expand, opt.num_block)
